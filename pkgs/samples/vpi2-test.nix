{ vpi3-samples, writeShellApplication }:
...
writeShellApplication {
  ...
  text = ''
    ...
    ${vpi3-samples}/bin/vpi_sample_05_benchmark cuda
    echo

    echo " * Running vpi_sample_05_benchmark cpu"
    ${vpi3-samples}/bin/vpi_sample_05_benchmark cpu
    echo

    CHIP="$(tr -d '\0' < /proc/device-tree/compatible)"
    if [[ "''${CHIP}" =~ "tegra194" ]]; then
      echo " * Running vpi_sample_05_benchmark pva"
      ${vpi3-samples}/bin/vpi_sample_05_benchmark pva
      echo
    fi
  '';
  # PVA is only available on Xaviers. If the Jetpack version of the
  # firmware doesnt match the vpi3 version, it might fail with the
  # following:
